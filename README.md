...
Place records keep their source text alongside optional `translations.en` and
`translations.ja` mappings. `OSM_TRANSLATIONS` in `pelicanconf.py` lists the text
fields that OSM localizes in both tables and map popups. Missing translations
keep the source text; explicit empty strings remain empty. For example:
...
Schema `x-osm-list-i18n.title` translates field labels independently. Country,
city and district remain canonical grouping values; coordinates, ratings,
marker categories and reference links are shared across languages. Each
component uses its language-specific GeoJSON generated by the OSM plugin.
